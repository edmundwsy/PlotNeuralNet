...
# define new block
def block_2ConvPool(
    name, botton, top, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32, 32, 3.5), opacity=0.5
):
    return [
        to_ConvConvRelu(
...
def block_ConvLSTM_Layer(
    name, botton, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32, 32, 3.5), opacity=0.5):
    return [
        to_Conv(
        ...
        to_ConvLSTM(
            name="clstm_{}".format(name),
            s_filer=str(s_filer),
            n_filer=(n_filer, n_filer),
            offset="(0,0,0)",
            to="(conv_{}-east)".format(name),
            width={size[2], size[2]},
            height=size[0],
            depth=size[1],
        ),
        to_connection("{}".format(botton), "conv_{}".format(name)),
    ]
